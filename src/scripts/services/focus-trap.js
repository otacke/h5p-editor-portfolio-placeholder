export default class FocusTrap {
  ...
  getFocusableElements(container) {
    ...
    return []
      ...
      .filter((element) => {
        // Check node chain up to container for hidden/disabled
        return [element, ...this.getParentNodes(element, container)]
          .every((element) => this.isElementFocusable(element));
      });
  }
   ...
   * @returns {boolean} True, if element can be focused.
   */
   ...
   * @param {HTMLElement|object} limitNode Node to stop looking at.
   * @returns {HTMLElement[]} Parent nodes.
   */
  getParentNodes(child, limitNode = {}) {
    const parents = [];
    while (child.parentNode && child.parentElement !== limitNode) {
